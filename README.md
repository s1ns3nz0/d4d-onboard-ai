...
이 프로젝트는 **두 개의 기둥**으로 구성된다.

### 1) 모든 비행체는 실물 없이 소프트웨어로 모사한다 (ArduPilot SITL)

실제 드론·하드웨어는 쓰지 않는다. 모든 air vehicle은 **ArduPilot SITL(Software-In-The-Loop)** 로
완전히 소프트웨어상에서 모사한다.

- ArduPilot SITL이 가상의 기체를 실제처럼 비행시키고, **MAVLink 텔레메트리**(위치·고도·헤딩)를 발행한다.
- 미션 웨이포인트를 따라 자동 비행하며, 각 프레임 시각에 `(lat, lon, alt, hdg)` 스냅샷을 남긴다.
- SITL에는 카메라 렌더가 없으므로 **영상은 공개 데이터셋/항공영상 리플레이**로 분리해 붙이고,
  텔레메트리 시각과 동기화해 **지오태깅**한다.
- 결과적으로 **하드웨어 0대, 100% 소프트웨어**로 "비행하는 정찰기"를 재현한다.

### 2) 온보드 AI는 Qwen 기반 — 객체 인식 + 텍스트 요약

기체에 탑재되는 온보드 AI의 **인식·요약 두 기능을 모두 Qwen 계열 모델로** 구현해 제출한다.

- **객체 인식**: **Qwen3-VL**(비전-언어 모델)로 항공 영상 속 객체(차량·선박·건물 등)를 인식한다.
  고해상(~4096) 입력을 지원해 항공 정찰 영상에 유리하다.
  *(빠른 회전 박스(OBB) 선탐지가 필요하면 YOLO-OBB를 경량 전처리로 앞단에 둘 수 있으나,
  인식·판단의 코어는 Qwen이다.)*
- **텍스트 요약**: **Qwen2.5/Qwen3 LLM**이 인식 결과 + 텔레메트리를 받아
  "객체 종류·개수·대략 위치"의 **구조화 상황 리포트**로 압축한다.
  (예: `T=00:12:03 POS=37.4825,127.0398 ALT=120 HDG=090 DET=[veh:3,ship:1] QUAD=NE`)
- 로컬 구동이 원칙(Ollama 서빙)이며, 데모 안정화가 필요하면 API 폴백을 둔다.

### 두 기둥이 만나는 지점 = 데모의 핵심

소프트웨어로 모사한 기체(①)가 비행하며 영상을 얻고, 온보드 Qwen AI(②)가 그 영상을
**텍스트 상황보고로 압축**한다. 그 압축된 보고가 링크 품질에 따라 단계적으로 폴백(L0→L3)하며
전달되기 때문에 — **"링크가 죽어도 상황 인식은 안 죽는다"** 는 한 줄 피치가 성립한다.
즉, 이 프로젝트는 **소프트웨어 모사 비행체 + Qwen 온보드 AI + 대역폭 적응형 통신**의 결합이다.
